...
class GridWorldMultipleAgent(pygame.sprite.Sprite, IAgent):
    def __init__(self, env, trainingMode=False, rootFol='', myid=0):
        super().__init__()
        self.agentsInfo = []
        self.env = env
    ...
    def update_agents_info(self):
        ...
        for agent in self.env.all_agent:
            ...
            if agent.Id == 0:
                c_x = self.rect.x + (self.env.sprite_width / 2)
                c_y = self.rect.y + (self.env.sprite_height / 2)
                t_cx = agent.rect.x + (self.env.sprite_width / 2)
                t_cy = agent.rect.y + (self.env.sprite_height / 2)
                pygame.draw.line(self.env.screen, (255, 255, 0), [c_x, c_y], [t_cx, t_cy], 2)
                distance = math.hypot((t_cx - c_x), (t_cy - c_y))
                angle = math.atan2(c_y - t_cy, c_x - t_cx)
                self.agentsInfo.append([distance, angle])
                return
            # print(self.targetsInfo)
    ...
    def observation(self, surface, view_range=5):
        ...
        for i in range(view_range):
            for j in range(view_range):
                ...
                agents = list(filter(lambda e: e.rect.x == xk and e.rect.y == yk, self.env.all_agent))
                sprite = sprites[len(sprites)-1] if len(sprites) > 0 else None
                agent = agents[len(agents)-1] if len(agents) > 0 else None
                val = 0
                if isinstance(sprite, Background):
                    ...
                    val = -1
                if agent is not None and agent.Id != self.Id:
                    val = 1
                res[j, i] = int(val)
                # if agent is not None and agent.Id != self.Id:
        ...
        if len(self.agentsInfo) > 0:
            result = np.concatenate([result, np.array(self.agentsInfo).flatten()])
        else:
            result = np.concatenate([result, np.array([-1, -1]).flatten()])
        # print(self.targetsInfo)
        # if len(result) != 102:
        #     print('!!!')
        #     print(self.targetsInfo)
    ...
    def move(self, direct):
        ...
        try:
            if self.finish:
                return True, 0
            if direct == 1:
                hit, step_reward = self.moveUp(self.height)
            elif direct == 2:
                hit, step_reward = self.moveRight(self.width)
            elif direct == 3:
                hit, step_reward = self.moveDown(self.width)
            elif direct == 4:
                hit, step_reward = self.moveLeft(self.height)
            self.update_agents_info()
            for dis, ang in self.agentsInfo:
                # print(dis)
                step_reward -= dis
                self.reward -= dis
            # print(step_reward)
            return hit, step_reward
